...
contract Insurance {
    ...
    function payOut(uint _policyId, uint _payout, string memory _claim) public
        ...
        uint temp1 = _policyId;
        uint temp2 = _payout;

        policy memory p = policies[temp1];
        require(p.state == policyState.Accepted);
        ...
        if (temp2 == 0) {
            ...
            p.stateTime = now;
            emit PolicyExpired(temp1);
        } else {

            p.calculatedPayout = temp2;
            ...
            p.actualPayout = temp2;

            if (!p.customer.send(temp2)){
                p.state = policyState.SendFailed;
                p.stateMessage = 'Payout, send failed!';
                p.actualPayout = 0;
                emit SendFail(temp1, 'payout sendfail');
            }
            else {
                ...
                p.stateTime = now;
				balance -= temp2;
                emit PolicyPaidOut(temp1, temp2);
            }
        }
    }
